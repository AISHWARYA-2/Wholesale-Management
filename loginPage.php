	...
	<div class="mybox" style="display:inline-block;">
		<div id="userPrompt" style="display:block;">
			<form method='post' action='login.php'>
				<h3>Login</h3>
				<input class="inputItem" type=text name='user' placeholder="Username" required><br><input class="inputItem" type=password name='pass' placeholder="Password" required><br><input type="submit" value="Login" class="btn loginbt">
			</form>
		</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#userLogin').click(function() {
			...
			$('#AdminPrompt').css("display", "none");
		});
		$('#Adminlogin').click(function() {
			$('#userPrompt').css("display", "none");
			$('#AdminPrompt').slideDown();
			$('#createAccountPrompt').css("display", "none");
		});
